...
const PartnerWithUsForm = () => {
  ...
  const submitFormData = async () => {
    ...
    if (response?.data?.status === 200) {
      ...
      snackbarEmitter(
        "Your form has been successfully submitted. You will receive a response from us shortly regarding your enquiry.",
        "success"
      );
    } else {
      setLoading(false);
          ...
          <Grid size={{ xs: 12 }} key={field.name}>
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
              <Typography
                ...
                fontWeight={500}
                sx={{
                  color: theme.palette.mode === "dark" ? "#FFFFFF" : "#767976",
                }}
              >
                {field.label}
              </Typography>
